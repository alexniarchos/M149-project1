...
@Component
class DataToSanitationValidationService(
  ...
  companion object {
    ...
    private const val TYPE_OF_VIOLATION = 5
  ...
  @Transactional
  fun convertAndSave(line: Array<String?>): SanitationViolation {
    ...
    return SanitationViolation(
      event.id!!,
      line[TYPE_OF_VIOLATION]
    )
  }
  ...
  fun convertAndSaveAsDump(line: Array<String?>): Dump =
    dataToDumpService.convertAndSave(
      ...
      wardsPosition = WARDS,
      typeOfViolationPosition = TYPE_OF_VIOLATION
    )
}
